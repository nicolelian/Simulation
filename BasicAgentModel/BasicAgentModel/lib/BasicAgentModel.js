...
const urlCustomer = "images/icon-customer.png";
const urlDarts = "images/icon-darts.png";
const urlCashier = "images/icon-cashier.png";
const urlDrinks = "images/icon-beer.png";
...
//a customer enters the hospital if EMPTY; he or she then is WAITING to be attended to by cashier; 
// then BUYING with the cashier; then EATING;
// When the customer has EATEN he or she leaves the cafe immediately at that point.
const EMPTY = 0;
const OCCUPIED = 1;
const WAITING= 1;
const STAGING= 2; 
...
// The cashier can be either BUSY attending to customer, or IDLE, waiting for a customer 
const IDLE = 0;
const BUSY = 1;

// There are three types of services in our system: cashier, drinks and darts
const CASHIER = 0;
const DRINKS = 1;
const DARTS = 2;

// customer is a dynamic list, initially empty
var customers = [];
var waitingSeats = [];
var seatState;	

// services is a static list, populated with a cashier,drinks and darts	
var services = [
    { "type": CASHIER ,"label":"Cashier","location":{"row": cashierRow,"col": cashierCol},"state":IDLE},
    { "type": DRINKS, "label": "Drinks", "location": { "row": drinksRow, "col": drinksCol }, "state": IDLE }
    { "type": DARTS, "label": "Darts", "location": { "row": dartsRow, "col": dartsCol }, "state": IDLE }
];
var cashier = services[0]; // the cashier is the first element of the services list.
var drinks = services[1]; // drinks second element of services list
var darts = services[2]; // darts third element of services list

// We can section our screen into different areas. In this model, the waiting area and the staging area are separate.
var areas = [
 { "label": "Sitting Area", "startRow": table1Row, "numRows": 2, "startCol": table1Col, "numCols": 2, "color": "black" },
 { "label": "Waiting Area", "startRow": cashierRow, "numRows": 1, "startCol": cashierCol - 4, "numCols": 4, "color": "blue" },
 {"label":"Staging Area","startRow":cashierRow,"numRows":1,"startCol":cashierCol-1,"numCols":1,"color":"red"}	
]
var sittingArea = areas[0]; // the sitting area is the first element of the areas array
...
function redrawWindow(){
    ...
    updateSurface();	
    waitingSeats = [];					//reset waiting Seats array 

    seatCount = 4
    for (i = 0; i < seatCount; i++) {			//creating an array to represents seats in room
        waitingSeats.push(i);
    }
    //initialize seats in room with its location + state
    waitingSeats = waitingSeats.map(function (d, i) {
        var state = EMPTY;
        var row = sittingArea.startRow + Math.floor(i / sittingArea.numCols);
        var col = sittingArea.startCol + i - (row - sittingArea.startRow) * sittingArea.numCols;
        return { 'row': row, 'col': col, 'state': state }
    })

    waitingSeats.push({ 'state': EMPTY })	//push in one extra space seat for redundancy for double counting in waiting-staging
    //seatState = waitingSeats.state;
...
function updateSurface(){
	...
	var newcustomer = allcustomers.enter().append("g").attr("class","customer"); 
	...
	newcustomer.append("svg:image")
	 ...
	 .attr("xlink:href",function(d){if (d.type=="A") return urlCustomer; else return urlCustomer;});
	...
	var allservices = surface.selectAll(".services").data(services);
	...
	var newservices = allservices.enter().append("g").attr("class","services");
	newservices.append("svg:image")
	 .attr("x",function(d){var cell= getLocationCell(d.location); return cell.x+"px";})
	 .attr("y",function(d){var cell= getLocationCell(d.location); return cell.y+"px";})
	 .attr("width", Math.min(cellWidth,cellHeight)+"px")
        .attr("height", Math.min(cellWidth, cellHeight) + "px")
        .attr("xlink:href", function (d) { if (d.type == CASHIER) return urlCashier; if (d.type == DRINKS) return urlDrinks; else return urlDarts });
	
	// It would be nice to label the caregivers, so we add a text element to each new caregiver group
	newservices.append("text")
...
function addDynamicAgents(){
	...
	if (Math.random()< probArrival){
		var newcustomer = {"id":1,"type":"A","location":{"row":20,"col":12},
		"target":{"row":cashierRow,"col":cashierCol},"state":WAITING,"timeAdmitted":0};
		...
		case STAGING:
            ...
            if (hasArrived) {
                ...
                if (doctor.state == IDLE) {
                    if (patient.type == 'A') var isnext = true; else isnext = false;

                    if (isnext == false) {
                        var nextpatientA = patients.filter(function (d) { return d.type == 'A' && d.id == nextTreatedPatientID_A && d.state == STAGING && d.row == (doctorRow - 1) && d.col == (doctorCol - 1) });
                        console.log(nextpatientA);
                        if (nextpatientA.length == 0) isnext = true;
                    }
