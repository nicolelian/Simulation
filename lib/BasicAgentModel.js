...
var doctorRow = 21;
var doctorCol = 20;
var receptionistRow = 15;
var receptionistCol = 40;
var cashierRow = 15;
var cashierCol = 39;
...
{"name":"Average percentage of patients rejected:","location":{"row":doctorRow+5,"col":doctorCol-4},"dischargeCount":0,"count":0}
];
...
(function() {
	...
	document.getElementById("title").textContent="The Crooked Ones"
	redrawWindow();
})();
...
function updateSurface(){
	...
	allstatistics.selectAll("text").text(function(d) {
		if (d.name == "Average percentage of patients rejected:"){
			var PercentReject = d.dischargeCount*100/Math.max(1,d.count);
			return d.name+PercentReject.toFixed(1);
		} else{
	...
	var allseats = surface.selectAll(".waitingSeats").data(waitingSeats);
	var newseats = allseats.enter().append("g").attr("class","waitingSeats");
...
function updatePatient(patientIndex){
	...
	switch(state){
		case UNTREATED:
			if (hasArrived){
				var emptySeats = waitingSeats.filter(function(d){
					return d.state==0;
				})
				
				console.log(emptySeats.length)
				if (emptySeats.length ==0){
					patient.state = DISCHARGED;
					statistics[2].dischargeCount++;
					statistics[2].count = statistics[1].count + statistics[0].count + statistics[2].dischargeCount;
				} else{
					var chosenSeat = emptySeats[Math.floor(Math.random()*emptySeats.length)]
					
					for(var j of waitingSeats){
						if(j.row == chosenSeat.row && j.col == chosenSeat.col){
							patient.target.row = j.row
							patient.target.col = j.col
							j.state = 1
						}
					}
				}
			...
			switch (type){
				case "A":
					if (patient.id == nextTreatedPatientID_A){
						...
						for (var k of waitingSeats){
							if (k.row == patient.target.row && k.col == patient.target.col){
								k.state = 0;
							}
						}
